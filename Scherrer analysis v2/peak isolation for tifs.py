### Run this script after running the Viewer for Integration to 1D.py script and getting isolated 1D curves. This will allow you to select a specific frame, perform background subtraction, and then isolate and fit peaks for further analysis.
# You will run this script in "ITO Calibration Mode" to refine your calibration parameters based on the ITO reference peaks first then select a sample from your dataset to analyze using the new calibration curves from the terminal output
...
# =========================================================================
# X-RAY CONVERSION CONSTANTS (Verify your experimental energy!)
# =========================================================================
ENERGY_KEV = 10.0
...
# =========================================================================
DEFAULT_ITO_CENTERS = [1.52301, 2.14004, 2.46856, 3.48597, 4.09467]
DEFAULT_ITO_FWHMS   = [0.02173, 0.03256, 0.03200, 0.04500, 0.04867]
DEFAULT_ITO_ETAS    = [0.00000, 0.11954, 0.00000, 0.00000, 0.00000]
...
def ask_operation_mode():
    ...
    response = messagebox.askyesno(
        ...
        "Click 'No' to analyze isolated sample data using pre-saved calibration curves."
    )
    root.destroy()
    return response
...
if IS_ITO_CALIBRATION:
    ...
    print(f"Using calibration with {len(ITO_CENTERS)} ITO reference peaks")
    Q_MIN = 0.90
    Q_MAX = 1.15
    TARGET_PEAKS = {
    ...
    if len(ITO_CENTERS) >= 3:
        fwhm_irf_curve = np.poly1d(np.polyfit(ITO_CENTERS, ITO_FWHMS, deg=2))
        eta_irf_curve = np.poly1d(np.polyfit(ITO_CENTERS, ITO_ETAS, deg=2))
    else:
        fwhm_irf_curve = np.poly1d(np.polyfit(ITO_CENTERS, ITO_FWHMS, deg=1))
        eta_irf_curve = np.poly1d(np.polyfit(ITO_CENTERS, ITO_ETAS, deg=1))
...
for center_guess in detected_peaks:
    ...
    if IS_ITO_CALIBRATION:
        ...
        fwhm_min = max(0.004, expected_inst_fwhm * 0.3)
...
if len(initial_guesses) > 2:
    try:
        weights = 1.0 / (np.sqrt(np.abs(y_subtracted_clean)) + 1.0)
        popt, _ = curve_fit(
...
# =========================================================================
# STAGE 4: ADVANCED DECONVOLUTION (SCHERRER ANALYSIS)
# =========================================================================
scherrer_results = {}

IRF_SCALE = 1.02
MICROSTRAIN_PCT = 0.25
...
if scherrer_results and not IS_ITO_CALIBRATION:
    ...
    for peak, data in scherrer_results.items():
        if data['is_limited']:
            print(f" Peak ({peak}) at q={data['center']:.3f} Å⁻¹ -> Size: ~{data['size_nm']:.1f} nm (Resolution Limit)")
        else:
...
if scherrer_results and not IS_ITO_CALIBRATION:
    ...
    msg = "ITO Calibration active. Reference profile metrics saved successfully." if IS_ITO_CALIBRATION else "No target perovskite phase peaks (100) detected."
